...
impl Action {
    ...
    pub fn keys(&self) -> &[Key] {
        match self {
            ...
            Action::UpdateOne => &[Key::Char('u')],
            Action::UpdateAll => &[Key::Char('U')],
        }
    }
}
...
impl Display for Action {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let str = match self {
            ...
            Action::UpdateOne => "Update highlighted pair",
            Action::UpdateAll => "Update all pairs",
        };
        write!(f, "{}", str)
    }
}
